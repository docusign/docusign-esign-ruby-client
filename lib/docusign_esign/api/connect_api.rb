...
module DocuSign_eSign
  ...
  class ConnectApi
    ...
    # @param account_id The external account number (int) or account ID Guid.
    # @return [nil]
    def get_connect_secret(account_id)
      get_connect_secret_with_http_info(account_id)
      return nil
    end
    ...
    # @param account_id The external account number (int) or account ID Guid.
    # @return [Array<(nil, Fixnum, Hash)>] nil, response status code and response headers
    def get_connect_secret_with_http_info(account_id)
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: ConnectApi.get_connect_secret ..."
      end
      # verify the required parameter 'account_id' is set
      fail ArgumentError, "Missing the required parameter 'account_id' when calling ConnectApi.get_connect_secret" if account_id.nil?
      # resource path
      local_var_path = "/v2.1/accounts/{accountId}/connect/secret".sub('{format}','json').sub('{' + 'accountId' + '}', account_id.to_s)
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :auth_names => auth_names)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ConnectApi#get_connect_secret\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
